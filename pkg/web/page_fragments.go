...
func pageStyles() string {
	return strings.Join([]string{
		...
		".button.compact{min-height:32px;padding:6px 9px;font-size:.84rem}",
		...
		".row-actions{display:flex;gap:6px;flex-wrap:wrap;min-width:230px}",
		...
		".json-panel{margin-top:10px;border:1px solid var(--line);border-radius:8px;background:#fff;overflow:hidden}",
		".json-panel-header{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 10px;border-bottom:1px solid var(--line);background:#f8fafc;flex-wrap:wrap}",
		".json-panel-title{font-weight:700;font-size:.88rem}",
		".json-panel-summary{color:var(--muted);font-size:.8rem;overflow-wrap:anywhere}",
		".result-box{margin:0;max-height:360px;overflow:auto;background:var(--code);color:#e4e7ec;padding:10px;font-size:.84rem;white-space:pre-wrap}",
...
func pageConsoleLayout(opts Options) string {
	return opts.ExtraSectionsHTML +
		...
		"<div class=\"subgrid\" style=\"margin-top:12px\" id=\"support-timeline-summary\"><div class=\"stat\"><span>Context</span><strong id=\"support-timeline-user-card\">pending</strong></div><div class=\"stat\"><span>Quota</span><strong id=\"support-timeline-quota-card\">pending</strong></div><div class=\"stat\"><span>Devices</span><strong id=\"support-timeline-device-card\">pending</strong></div><div class=\"stat\"><span>Timeline</span><strong id=\"support-timeline-event-card\">pending</strong></div></div>" +
		"<div class=\"panel\" style=\"margin-top:12px\" id=\"support-timeline-repair\"><div class=\"section-header\"><h3>Repair actions</h3><span class=\"pill\" id=\"support-timeline-repair-status\">idle</span></div><div class=\"action-row\"><button class=\"button\" id=\"recalculate-support-quota\" type=\"button\" disabled>Recalculate quota</button><button class=\"button secondary\" id=\"refresh-support-context-after-action\" type=\"button\" disabled>Refresh context</button></div><div class=\"detail-grid\" id=\"support-timeline-repair-summary\" style=\"margin-top:10px\"><div class=\"detail-item\"><span>Selected user</span><strong>none</strong></div></div><div class=\"json-panel\"><div class=\"json-panel-header\"><span class=\"json-panel-title\" id=\"support-timeline-repair-result-title\">Repair result</span><span class=\"json-panel-summary\" id=\"support-timeline-repair-result-summary\">No action yet</span></div><pre class=\"result-box\" id=\"support-timeline-repair-result\">{}</pre></div></div>" +
		"<div class=\"split-panel\" style=\"margin-top:12px\"><div class=\"panel\"><h3>Incident summary</h3><div class=\"detail-grid\" id=\"support-timeline-detail-grid\"></div><div class=\"json-panel\"><div class=\"json-panel-header\"><span class=\"json-panel-title\" id=\"support-timeline-json-title\">Timeline context JSON</span><span class=\"json-panel-summary\" id=\"support-timeline-json-summary\">No lookup yet</span></div><pre class=\"result-box\" id=\"support-timeline-json\">{}</pre></div></div><div class=\"panel\"><h3>Recent operations</h3><div class=\"table-wrap\"><table><thead><tr><th>Created</th><th>Action</th><th>Category</th><th>Target</th></tr></thead><tbody id=\"support-timeline-action-rows\"><tr><td colspan=\"4\" class=\"muted\">no lookup</td></tr></tbody></table></div><div class=\"table-wrap\" style=\"margin-top:12px\"><table><thead><tr><th>Updated</th><th>Job</th><th>Status</th><th>Detail</th></tr></thead><tbody id=\"support-timeline-job-rows\"><tr><td colspan=\"4\" class=\"muted\">no lookup</td></tr></tbody></table></div></div></div></section>" +
		...
		"<section class=\"section\" id=\"ops-checks\"><div class=\"section-header\"><h2>Ops checks</h2><button class=\"button secondary\" id=\"refresh-ops\" type=\"button\">Refresh ops</button></div><div class=\"subgrid\" id=\"ops-summary\"><div class=\"stat\"><span>Dependency check</span><strong>pending</strong></div><div class=\"stat\"><span>Consistency check</span><strong>pending</strong></div></div><div class=\"two-col\" style=\"margin-top:12px\"><div class=\"panel\"><h3>Recent runs</h3><div class=\"table-wrap\"><table><thead><tr><th>Started</th><th>Type</th><th>Status</th><th>Artifacts</th></tr></thead><tbody id=\"ops-run-rows\"><tr><td colspan=\"4\" class=\"muted\">pending</td></tr></tbody></table></div></div><div class=\"panel\"><h3>Recent failures</h3><div class=\"table-wrap\"><table><thead><tr><th>Started</th><th>Type</th><th>Status</th><th>Summary</th></tr></thead><tbody id=\"ops-failure-rows\"><tr><td colspan=\"4\" class=\"muted\">pending</td></tr></tbody></table></div></div></div><div style=\"margin-top:14px\" id=\"ops-actions\"><div class=\"section-header\"><h3>Ops actions</h3><span class=\"pill\" id=\"ops-action-status\">idle</span></div><div class=\"compact-form\"><button class=\"button\" id=\"run-dependency-check\" type=\"button\">Run dependency check</button><div><label for=\"ops-consistency-limit\">Consistency limit</label><input id=\"ops-consistency-limit\" type=\"number\" min=\"1\" max=\"5000\" step=\"1\" value=\"1000\"></div><button class=\"button\" id=\"run-consistency-check\" type=\"button\">Run consistency check</button></div><div class=\"split-panel\" style=\"margin-top:12px\"><form id=\"restore-rehearsal-form\" class=\"panel\"><h3>Restore rehearsal</h3><div class=\"compact-form\"><div><label for=\"restore-rehearsal-mode\">Mode</label><select id=\"restore-rehearsal-mode\" name=\"mode\"><option value=\"baseline\">Generate restore baseline</option><option value=\"verify\">Verify restore manifest</option></select></div><div><label for=\"restore-rehearsal-limit\">Limit</label><input id=\"restore-rehearsal-limit\" name=\"limit\" type=\"number\" min=\"1\" max=\"5000\" step=\"1\" value=\"1000\"></div><button class=\"button\" type=\"submit\">Run restore rehearsal</button></div><label for=\"restore-manifest-json\" style=\"margin-top:10px\">Manifest JSON</label><textarea id=\"restore-manifest-json\" name=\"manifest\" spellcheck=\"false\" placeholder=\"Paste the manifest field or a previous restore rehearsal response for verify mode\"></textarea></form><div class=\"panel\"><h3>Downloads</h3><form class=\"compact-form\" id=\"support-bundle-form\"><div><label for=\"support-bundle-since\">Support bundle since</label><input id=\"support-bundle-since\" name=\"since\" placeholder=\"optional RFC3339\"></div><button class=\"button secondary\" type=\"submit\">Download support bundle</button></form><form class=\"compact-form\" id=\"operational-export-form\" style=\"margin-top:12px\"><div><label for=\"export-record-type\">Record type</label><select id=\"export-record-type\" name=\"record_type\"><option value=\"audit_logs\">audit_logs</option><option value=\"ops_history\">ops_history</option><option value=\"notification_outbox\">notification_outbox</option></select></div><div><label for=\"export-format\">Format</label><select id=\"export-format\" name=\"format\"><option value=\"json\">json</option><option value=\"csv\">csv</option></select></div><div><label for=\"export-source\">Source</label><select id=\"export-source\" name=\"source\"><option value=\"hot\">hot</option><option value=\"archive\">archive</option><option value=\"all\">all</option></select></div><div><label for=\"export-limit\">Limit</label><input id=\"export-limit\" name=\"limit\" type=\"number\" min=\"1\" max=\"5000\" step=\"1\" value=\"1000\"></div><div><label for=\"export-from\">From</label><input id=\"export-from\" name=\"from\" placeholder=\"optional RFC3339\"></div><div><label for=\"export-to\">To</label><input id=\"export-to\" name=\"to\" placeholder=\"optional RFC3339\"></div><button class=\"button secondary\" type=\"submit\">Export operational records</button></form></div></div><div class=\"detail-grid\" id=\"ops-action-summary\" style=\"margin-top:12px\"><div class=\"detail-item\"><span>Last action</span><strong>none</strong></div></div><div class=\"json-panel\"><div class=\"json-panel-header\"><span class=\"json-panel-title\" id=\"ops-action-result-title\">Action result</span><span class=\"json-panel-summary\" id=\"ops-action-result-summary\">No action yet</span></div><pre class=\"result-box\" id=\"ops-action-result\">{}</pre></div></div></section>" +
		"<section class=\"section\" id=\"notification-failures\"><div class=\"section-header\"><h2>Notification failures</h2><div class=\"action-row\"><button class=\"button secondary\" id=\"retry-visible-notifications\" type=\"button\" disabled>Retry visible failures</button><button class=\"button secondary\" id=\"refresh-notifications\" type=\"button\">Refresh notifications</button></div></div><div class=\"table-wrap\"><table><thead><tr><th>Created</th><th>User</th><th>Channel</th><th>Category</th><th>Type</th><th>Attempts</th><th>Error</th><th>Actions</th></tr></thead><tbody id=\"notification-failure-rows\"><tr><td colspan=\"8\" class=\"muted\">pending</td></tr></tbody></table></div></section>" +
